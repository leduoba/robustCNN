function Y = vl_nnhybridloss(X,c,para,dzdy)
...
% para: [alpha_1, alpha_2, alpha_3, p];
tol = eps; alpha_1 = para(1); alpha_2 = para(2); alpha_3 = para(3); p = para(4);
if nargin <= 3
  t1 = log(sum(X,3) + tol) - reshape(log(X(c_) + tol), [sz(1:2) 1 sz(4)]) ;
  t2 = reshape((1 - X(c_)) .^ p, [sz(1:2) 1 sz(4)]) ;
  t3 = sum(X .^ p, 3) - reshape(X(c_) .^ p, [sz(1:2) 1 sz(4)]);
  t = alpha_1 * t1 + alpha_2 * t2 + alpha_3 * t3;
  Y = sum(sum(sum(mass .* t,1),2),4) ;
else
  Y1 = repmat(1 ./ (sum(X, 3) + tol), [1, 1, sz(3), 1]); Y1(c_) = Y1(c_) - 1 ./ (X(c_) + tol);
  Y2 = zeros(sz, 'single'); Y2(c_) = - p * ((1 - X(c_)) .^ (p - 1));
  Y3 = p * X .^ (p - 1); Y3(c_) = 0;
  Y = alpha_1 * Y1 + alpha_2 * Y2 + alpha_3 * Y3;
