...
for i=1:n
  ...
  switch l.type
    ...
    case 'max'
      res(i+1).x = vl_nnmax(res(i).x) ;

    case 'rbf1d'
      res(i+1).x = vl_nnrbf1d(res(i).x) ;

    case 'mrelu'
      res(i+1).x = vl_nnmrelu(res(i).x) ;
    ...
    case 'hybridloss'
      if ~isfield(l, 'para'), para = [1 1 1 2]; else para = l.para; end
      res(i+1).x = vl_nnhybridloss(res(i).x, l.class, para) ;
    ...
    case 'nonzero'
      res(i+1).x = vl_nnnonzero(res(i).x) ;
  ...
  forget = forget & (~strcmp(l.type, 'hybridloss'));
  if forget
    res(i).x = [] ;
  end
...
if doder
  ...
  for i=n:-1:max(1, n-opts.backPropDepth+1)
    ...
    switch l.type
      ...
      case 'max'
        res(i).dzdx = vl_nnmax(res(i).x, res(i+1).dzdx) ;

      case 'rbf1d'
        res(i).dzdx = vl_nnrbf1d(res(i).x, res(i+1).dzdx) ;

      case 'mrelu'
        res(i).dzdx = vl_nnmrelu(res(i).x, res(i+1).dzdx) ;

      case 'nonzero'
        res(i).dzdx = vl_nnnonzero(res(i).x, res(i+1).dzdx) ;
      ...
      case 'hybridloss'
        if ~isfield(l, 'para'), para = [1 1 1 2]; else para = l.para; end
        res(i).dzdx = vl_nnhybridloss(res(i).x, l.class, para, res(i+1).dzdx) ;
